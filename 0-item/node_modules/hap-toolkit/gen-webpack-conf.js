...
const { colorconsole } = require('./packager/lib/common/utils')
const info = require('./packager/lib/common/info')
// 暂时暴露全局
...
module.exports = function genWebpackConf (options, mode) {
  ...
  function loadWebpackConfList () {
    const moduleList = [{
      name: 'packager',
      path: path.resolve(__dirname, './packager/lib/webpack.config.js')
    }, {
      // 增加：开发者项目目录下的config文件夹
      // 注意：Hook机制不保证向后兼容
      name: '',
      path: path.join(pathProject, 'config/webpack.config.js')
    }]
